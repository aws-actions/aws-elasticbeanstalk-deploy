...
`build-configuration` takes a JSON object with the fields of the `Build` member of the Elastic Beanstalk `ImageConfiguration` API. The action validates only that `Type` and `CodeBuildServiceRole` are present; the service validates the values. Fields that the AWS SDK bundled with the action does not model are not sent, and the action logs a warning naming them. The fields (see [Building container images for Beanstalk Cluster environments](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/beanstalk-cluster-app-versions.html)) are:
...
| Before deploying to an existing environment | No tier check | The environment's tier is checked first; mismatched inputs (or an unreadable tier) fail before anything is packaged or created |
